...
class EVADataset(Dataset):
    def __init__(self, args, tokenizer: EVATokenizer, dialog_path, split, ratio=1, num=-1, cache_path=None):
        ...
        if cache_path is None or not os.path.exists(os.path.join(cache_path, split + ".pkl")):
            ...
            self.contexts, self.targets, self.labels = self.preprocess(dialog_path)
        ...
        print_str = "Path: {} | Ratio:{} | Max enc len: {} | Max dec len: {} | Data num: {}".format(dialog_path, ratio,
                                                                                                    self.max_enc_len,
                                                                                                    self.max_dec_len,
                                                                                                    len(self.contexts))
        print(print_str)
        ...
        for line in tqdm(lines[:int(self.ratio * len(lines))], desc="Loading data from {}".format(path), disable=False):
            ...
            for c in context[::-1]:
                if len(c) + len(trunc_context) + 1 + 1 <= self.max_enc_len:  # first 1 for <sep>, second 1 for <s_0>
                    ...
                    break
            if len(trunc_context) > 0 and len(target)+2 <= self.max_dec_len:
    ...
    def collate(self, samples):
        ...
        batch = {
            ...
            "decoder_attention_mask": torch.zeros(bs, self.max_dec_len, self.max_dec_len),
            "encoder_attention_mask": torch.zeros(bs, self.max_dec_len, self.max_enc_len),  # cross attention mask
        ...
        for b in range(bs):
            ...
            batch["decoder_attention_mask"][b, :label_length - 1, :label_length - 1] = torch.tril(
                torch.ones(label_length - 1, label_length - 1))
            batch["encoder_attention_mask"][b, :label_length - 1, :context_length] = 1  # cross attention mask
            # batch["loss_ids"] = batch["decoder_attention_mask"]
        ...
        if self.args.fp16:
            ...
            # batch["loss_ids"] = batch["loss_ids"].half()
        return batch
